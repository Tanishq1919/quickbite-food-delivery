/* ============================================================
   QuickBite – app.js  (Firebase Firestore edition)
   ============================================================
   HOW TO ACTIVATE:
   1. Follow FIREBASE_SETUP.md to create your project
   2. Replace the firebaseConfig object below with YOUR config
   3. Open index.html — all data now persists in Firestore
   ============================================================ */
...
const firebaseConfig = {
  apiKey:            "AIzaSyB39obwM4Kpt5zgd4g_lwjjAzdxNXMLveo",
  authDomain:        "quickbite-62585.firebaseapp.com",
  projectId:         "quickbite-62585",
  storageBucket:     "quickbite-62585.firebasestorage.app",
  messagingSenderId: "278759173394",
  appId:             "1:278759173394:web:0e5438371bf118a17a97cb"
};
...
const DEFAULT_MENU = [
  ...
  { id:13, name:'BBQ Chicken Pizza',      desc:'Grilled chicken, BBQ sauce, red onion, jalapenos.',    price:429, emoji:'🍕', category:'pizza',    rating:4.7, reviews:240, restId:'pizza-grove'    },
...
/* ══════════════════════════════════
   SEED MENU (first time only)
══════════════════════════════════ */
async function seedMenuIfEmpty() {
  try {
    const snap = await menuCol.limit(1).get();
    if (!snap.empty) return;
    showToast('Setting up database for first time...');
    const batch = db.batch();
    DEFAULT_MENU.forEach(item => {
      batch.set(menuCol.doc(String(item.id)), { ...item, available: true });
    });
    await batch.commit();
    showToast('Database ready!');
  } catch(e) {
    console.warn('Seed failed (check Firebase config):', e.message);
  }
}
...
function startListeners() {
  ordersCol.orderBy('time', 'desc').onSnapshot(snap => {
    ORDERS_DB = snap.docs.map(d => {
      const data = d.data();
      return { ...data, time: data.time?.toDate ? data.time.toDate() : new Date(data.time) };
    });
    if (currentView === 'manager') { renderOverview(); renderOrdersTable(ordersFilter); }
    if (currentView === 'waiter')  renderWaiterOrders();
  }, err => console.warn('Orders listener error:', err.message));

  complaintsCol.orderBy('time', 'desc').onSnapshot(snap => {
    COMPLAINTS_DB = snap.docs.map(d => {
      const data = d.data();
      return { ...data, time: data.time?.toDate ? data.time.toDate() : new Date(data.time) };
    });
    if (currentView === 'manager') renderManagerComplaints();
    renderMyComplaints();
  }, err => console.warn('Complaints listener error:', err.message));

  menuCol.orderBy('id').onSnapshot(snap => {
    MENU = snap.docs.map(d => ({ ...d.data(), firestoreId: d.id }));
    renderMenu();
    if (currentView === 'manager') renderMenuManager();
  }, err => console.warn('Menu listener error:', err.message));
}
...
async function saveOrderToDB(order) {
  await ordersCol.doc(order.id).set({ ...order, time: firebase.firestore.Timestamp.now() });
}
async function updateOrderStatusDB(id, status) {
  await ordersCol.doc(id).update({ status });
}
async function saveComplaintToDB(c) {
  await complaintsCol.doc(c.id).set({ ...c, time: firebase.firestore.Timestamp.now() });
}
async function resolveComplaintDB(id, response) {
  await complaintsCol.doc(id).update({ resolved: true, response });
}
async function updateMenuItemDB(firestoreId, updates) {
  await menuCol.doc(firestoreId).update(updates);
}
async function deleteMenuItemDB(firestoreId) {
  await menuCol.doc(firestoreId).delete();
}
async function saveUserToDB(user) {
  if (!user || user.guest) return;
  const id = user.phone || ('user-' + Date.now());
...
function submitStaffLogin() {
  if (!selectedStaffRole) { showToast('Select a role first'); return; }
  if (document.getElementById('staff-pass').value.trim() !== '1234') { showToast('Wrong password. Demo: 1234'); return; }
  closeStaffLogin();
  if (selectedStaffRole === 'manager') {
    document.getElementById('manager-dashboard').style.display = 'block';
    currentView = 'manager'; initManagerDash();
  } else {
    document.getElementById('waiter-dashboard').style.display = 'block';
    currentView = 'waiter'; initWaiterDash();
  }
}
function closeStaffLogin() { document.getElementById('staff-login-modal').classList.remove('open'); }
...
/* ══════════════════════════
   INIT
══════════════════════════ */
function init() { renderRestaurants(); renderCategories(); renderMenu(); }
async function appStart() { await seedMenuIfEmpty(); startListeners(); }
  ...
  grid.innerHTML = RESTAURANTS.map((r,i) => `
    <div class="restaurant-card" style="animation-delay:${i*0.07}s" onclick="scrollToMenuAndFilter('${r.id}')">
      ...
      <div class="rest-body">
        ...
        <div class="rest-meta"><span>⭐ ${r.rating}</span><span>🕐 ${r.time} min</span><span>🚚 FREE</span></div>
...
function scrollToMenuAndFilter() { document.getElementById('menu').scrollIntoView({behavior:'smooth'}); }
...
function renderCategories() {
  ...
  el.innerHTML = CATEGORIES.map(c => `
    <div class="cat-chip ${c.id===activeCategory?'active':''}" onclick="selectCategory('${c.id}')">
      <span>${c.icon}</span>${c.label}
    </div>`).join('');
}
function selectCategory(id) { activeCategory=id; renderCategories(); renderMenu(); }
...
function renderMenu() {
  ...
  if (!grid) return;
  if (!MENU.length) { grid.innerHTML=`<p style="color:var(--muted);text-align:center;grid-column:1/-1;padding:40px 0">Loading menu...</p>`; return; }
  const items = MENU.filter(item => {
    if (item.available===false) return false;
    const mc = activeCategory==='all' || item.category===activeCategory;
    const ms = item.name.toLowerCase().includes(searchQuery) || item.desc.toLowerCase().includes(searchQuery);
    return mc && ms;
  });
  if (!items.length) { grid.innerHTML=`<p style="color:var(--muted);text-align:center;grid-column:1/-1;padding:40px 0">No items found. Try a different search!</p>`; return; }
  grid.innerHTML = items.map((item,i) => {
    const rest = RESTAURANTS.find(r=>r.id===item.restId);
    return `
      <div class="food-card" style="animation-delay:${i*0.06}s">
        <div class="food-img">${item.emoji}<span class="food-rest-badge">${rest?rest.emoji+' '+rest.name:''}</span></div>
        <div class="food-body">
...
function addToCart(id) {
  ...
  updateCart(); showToast(`${item.emoji} ${item.name} added!`);
}
...
function updateCart() {
  ...
  const s=document.getElementById('subtotal'); if(s) s.textContent=`₹${subtotal.toFixed(2)}`;
  const t=document.getElementById('total');    if(t) t.textContent=`₹${subtotal.toFixed(2)}`;
  const b=document.getElementById('checkout-btn'); if(b) b.disabled=!cart.length;
  const list=document.getElementById('cart-items-list'); if(!list) return;
  if (!cart.length) { list.innerHTML=`<div class="cart-empty"><div class="icon">🛒</div><p>Your cart is empty.<br>Add something tasty!</p></div>`; return; }
...
function handleCheckout() {
  toggleCart();
  if (!currentUser) setTimeout(()=>openSignIn(true),350);
  else setTimeout(()=>openPayment(),350);
}

/* ══════════════════════════
   SIGN IN
══════════════════════════ */
function openSignIn(fromCheckout=false) { _checkoutAfterSignIn=fromCheckout; showSignInStep1(); document.getElementById('signin-modal').classList.add('open'); }
...
function verifyOTP() {
  ...
  if (entered.length<4)        { showToast('Please enter all 4 digits'); return; }
...
function quickGuestLogin() {
  currentUser={guest:true,name:'Guest'}; updateNavUser(); closeSignIn(); showToast('Continuing as Guest!');
...
async function saveAddress() {
  ...
  if (!name||!street||!city||!pin) { showToast('Please fill in all fields'); return; }
  const phone=document.getElementById('phone-input').value.trim();
  currentUser={name,phone,address:`${street}, ${city} - ${pin}`};
  await saveUserToDB(currentUser);
  updateNavUser(); closeSignIn(); showToast(`Welcome, ${name}!`);
  if (_checkoutAfterSignIn) setTimeout(()=>openPayment(),400);
}
function updateNavUser() {
  const label=document.getElementById('nav-user-label'); const btn=document.getElementById('nav-sign-btn');
  if (!label) return;
  if (currentUser) { label.textContent=currentUser.guest?'Guest':currentUser.name.split(' ')[0]; btn.classList.add('signed-in'); }
  else { label.textContent='Sign In'; btn.classList.remove('signed-in'); }
}
...
function openPayment() {
  selectedPayMethod=null; selectedPayTiming=null;
  ['upi','card','wallet'].forEach(m=>{
    const el=document.getElementById(`check-${m}`); if(el) el.className='pay-check';
    const btn=document.getElementById(`popt-${m}`); if(btn) btn.classList.remove('selected');
  });
...
function showPayTiming() {
  ['pay-step-timing','pay-step-1','pay-step-cod-confirm','pay-step-upi','pay-step-card','pay-step-wallet']
    .forEach(id=>{const el=document.getElementById(id);if(el)el.style.display=id==='pay-step-timing'?'block':'none';});
}
...
function showPayStep1() {
  if (selectedPayTiming==='delivery') {
    document.getElementById('cod-amount-text').textContent=document.getElementById('total').textContent;
    ['pay-step-timing','pay-step-1','pay-step-upi','pay-step-card','pay-step-wallet'].forEach(id=>{const el=document.getElementById(id);if(el)el.style.display='none';});
    document.getElementById('pay-step-cod-confirm').style.display='block';
  } else {
    ['pay-step-timing','pay-step-cod-confirm','pay-step-upi','pay-step-card','pay-step-wallet'].forEach(id=>{const el=document.getElementById(id);if(el)el.style.display='none';});
...
function selectPayment(method) {
  ...
  ['upi','card','wallet'].forEach(m=>{
    const ck=document.getElementById(`check-${m}`); if(ck) ck.className=m===method?'pay-check checked':'pay-check';
    const bt=document.getElementById(`popt-${m}`);  if(bt) bt.classList.toggle('selected',m===method);
  });
...
function showPaymentDetails() {
  ...
  ['pay-step-1','pay-step-upi','pay-step-card','pay-step-wallet'].forEach(id=>{const el=document.getElementById(id);if(el)el.style.display='none';});
  if (selectedPayMethod==='upi')    { document.getElementById('upi-amount').textContent=total;    document.getElementById('pay-step-upi').style.display='block'; }
  else if (selectedPayMethod==='card')   { document.getElementById('card-amount').textContent=total;   document.getElementById('pay-step-card').style.display='block'; }
  else if (selectedPayMethod==='wallet') { document.getElementById('wallet-amount').textContent=total; document.getElementById('pay-step-wallet').style.display='block'; }
}
async function processPayment() {
  if (selectedPayTiming==='now') {
    if (selectedPayMethod==='upi' && !document.getElementById('upi-input').value.includes('@')) { showToast('Enter a valid UPI ID'); return; }
    if (selectedPayMethod==='card') {
      const num=document.getElementById('card-num').value.replace(/\s/g,'');
      if (!document.getElementById('card-name').value.trim()||num.length<16||document.getElementById('card-exp').value.length<5||document.getElementById('card-cvv').value.length<3) { showToast('Please fill in all card details'); return; }
    }
    if (selectedPayMethod==='wallet' && document.getElementById('wallet-input').value.trim().length<10) { showToast('Enter your 10-digit Paytm number'); return; }
  }
  showToast('Placing order...'); closePayment();
  setTimeout(async()=>{
    const subtotal=cart.reduce((s,c)=>s+c.price*c.qty,0);
    const methodLabel=selectedPayTiming==='delivery'?'Cash on Delivery':{upi:'UPI',card:'Card',wallet:'Paytm Wallet'}[selectedPayMethod];
    const name=currentUser&&!currentUser.guest?currentUser.name:'Guest';
    const addr=currentUser?.address||'';
    currentOrderId='QB'+String(Math.floor(Math.random()*90000)+10000);
    const orderData={
      id:currentOrderId, customer:name, phone:currentUser?.phone||'', address:addr,
      items:cart.map(c=>({id:c.id,name:c.name,emoji:c.emoji,price:c.price,qty:c.qty,restId:c.restId})),
      restaurant:[...new Set(cart.map(c=>RESTAURANTS.find(r=>r.id===c.restId)?.name||'QuickBite'))].join(', '),
      total:subtotal, payMethod:methodLabel, payTiming:selectedPayTiming,
      status:'pending', isRepeat:Math.random()>0.5
    };
    try {
      await saveOrderToDB(orderData);
    } catch(e) { console.error('Order save failed:',e); }
    document.getElementById('success-msg').innerHTML=
      `Hey ${name}, order <strong>${currentOrderId}</strong> of <strong>₹${subtotal.toFixed(2)}</strong> via <strong>${methodLabel}</strong> confirmed!`+
      (addr?`<br><small>Delivering to: ${addr}</small>`:'');
    document.getElementById('success-modal').classList.add('open');
    cart=[]; updateCart();
  },1200);
}
function closeSuccess()         { document.getElementById('success-modal').classList.remove('open'); }
function closeSuccessAndTrack() { closeSuccess(); setTimeout(()=>openTracking(),300); }
...
function openTracking() {
  if (!currentOrderId) { showToast('No active order to track'); return; }
  document.getElementById('track-order-id').textContent=`Order #${currentOrderId}`;
  let step=0; updateTrackingUI(step);
  document.getElementById('tracking-modal').classList.add('open');
  clearInterval(trackingInterval);
  trackingInterval=setInterval(()=>{
    step=Math.min(step+1,3); updateTrackingUI(step);
    const statuses=['pending','preparing','out','delivered'];
    updateOrderStatusDB(currentOrderId,statuses[step]).catch(()=>{});
    if (step===3) clearInterval(trackingInterval);
  },6000);
}
function updateTrackingUI(activeStep) {
  const pcts=[10,35,70,100]; document.getElementById('track-progress-fill').style.width=pcts[activeStep]+'%';
  document.getElementById('track-eta-text').textContent=['35 min','28 min','12 min','Delivered!'][activeStep];
  document.getElementById('tracking-steps').innerHTML=TRACKING_STEPS_DEF.map((s,i)=>{
    const isDone=i<activeStep, isActive=i===activeStep;
    return `<div class="track-step">
      <div class="track-step-icon ${isDone?'done':isActive?'active':''}">${isDone?'✓':s.icon}</div>
...
function closeTracking() { document.getElementById('tracking-modal').classList.remove('open'); clearInterval(trackingInterval); }
...
function switchHelpTab(tab,el) {
  ...
  el.classList.add('active'); document.getElementById('help-'+tab).classList.add('active');
}
function toggleFaq(el) {
  const a=el.nextElementSibling; a.classList.toggle('open');
...
async function submitComplaint() {
  ...
  if (!type||!text) { showToast('Please fill in type and description'); return; }
  const complaint={
    id:'CMP'+Date.now(), type, orderId:orderId||'N/A', text,
    customer:currentUser?(currentUser.guest?'Guest':currentUser.name):'Anonymous',
    phone:currentUser?.phone||'', resolved:false, response:''
  };
  try {
    ...
    renderMyComplaints();
  } catch(e) { showToast('Could not save complaint — check Firebase config'); }
}
function renderMyComplaints() {
  const list=document.getElementById('my-complaints-list'); if(!list) return;
  const mine=COMPLAINTS_DB.filter(c=>currentUser&&!currentUser.guest&&c.phone&&c.phone===currentUser.phone);
  if (!mine.length) { list.innerHTML=`<div style="text-align:center;color:var(--muted);padding:40px 0">No complaints yet.</div>`; return; }
  list.innerHTML=mine.map(c=>`
    <div class="complaint-card" style="margin-bottom:14px">
      ...
      <div>${c.resolved?`<span class="resolved-badge">Resolved</span>${c.response?`<p style="color:var(--muted);font-size:.82rem;margin-top:8px">${c.response}</p>`:''}`:
        '<span style="color:var(--muted);font-size:.82rem">Under review</span>'}</div>
    </div>`).join('');
}
...
function initManagerDash() {
  document.getElementById('dash-date-str').textContent=new Date().toLocaleDateString('en-IN',{weekday:'long',year:'numeric',month:'long',day:'numeric'});
...
function switchDashTab(tab) {
  ...
  document.querySelectorAll('.dash-tab')[tabs.indexOf(tab)].classList.add('active');
...
function renderOverview() {
  ...
  const repeatRate=ORDERS_DB.length?Math.round(ORDERS_DB.filter(o=>o.isRepeat).length/ORDERS_DB.length*100):0;
  const kg=document.getElementById('kpi-grid');
  if(kg) kg.innerHTML=`
    <div class="kpi-card gold"><div class="kpi-label">Today's Revenue</div><div class="kpi-value">₹${todayRev.toLocaleString('en-IN')}</div><div class="kpi-sub">From ${todayOrders.length} orders today</div></div>
  ...
  el('cust-today',todayOrders.length); el('cust-repeat',repeatCust);
  el('cust-new',ORDERS_DB.length-repeatCust); el('repeat-pct',repeatRate+'%');
  setTimeout(()=>{const c=document.getElementById('repeat-ring-circle');if(c)c.style.strokeDashoffset=String(314*(1-repeatRate/100));},300);
  renderBarChart('rev-bar-chart');
  const oo=document.getElementById('overview-orders');if(oo)oo.innerHTML=renderOrderRows(ORDERS_DB.slice(0,5));
}
function renderBarChart(containerId) {
  const days=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
  const totals=[0,0,0,0,0,0,0];
  ORDERS_DB.forEach(o=>{if(o.status!=='cancelled'){const d=new Date(o.time).getDay();totals[d===0?6:d-1]+=o.total;}});
  const max=Math.max(...totals,1);
  const c=document.getElementById(containerId);if(!c)return;
  c.innerHTML=totals.map((v,i)=>`
    <div class="bar-wrap">
      <div class="bar-val">₹${v>0?(v/1000).toFixed(1)+'k':'0'}</div>
      <div class="bar" style="height:${Math.max((v/max)*100,3)}%"></div>
...
function renderRevenue() {
  const todayOrders=ORDERS_DB.filter(o=>isToday(o.time));
  const todayRev=todayOrders.filter(o=>o.status!=='cancelled').reduce((s,o)=>s+o.total,0);
  const allRev=ORDERS_DB.filter(o=>o.status!=='cancelled').reduce((s,o)=>s+o.total,0);
  const avg=ORDERS_DB.length?Math.round(allRev/ORDERS_DB.length):0;
  const rk=document.getElementById('rev-kpi-grid');
  if(rk)rk.innerHTML=`
    <div class="kpi-card gold"><div class="kpi-label">Today</div><div class="kpi-value">₹${todayRev.toLocaleString('en-IN')}</div><div class="kpi-sub">${todayOrders.length} orders</div></div>
    <div class="kpi-card green"><div class="kpi-label">All Time</div><div class="kpi-value">₹${(allRev/1000).toFixed(1)}k</div><div class="kpi-sub">Total revenue</div></div>
    <div class="kpi-card blue"><div class="kpi-label">Avg Order Value</div><div class="kpi-value">₹${avg}</div><div class="kpi-sub">Per transaction</div></div>
  ...
  const byRest={};
  ORDERS_DB.forEach(o=>{if(o.status!=='cancelled'){if(!byRest[o.restaurant])byRest[o.restaurant]=0;byRest[o.restaurant]+=o.total;}});
  const sorted=Object.entries(byRest).sort((a,b)=>b[1]-a[1]);
  const maxR=sorted[0]?.[1]||1;
  const rb=document.getElementById('rev-by-rest');
  if(rb)rb.innerHTML=sorted.slice(0,6).map(([name,rev])=>`
    <div style="margin-bottom:14px">
      ...
      <div class="track-progress"><div class="track-progress-fill" style="width:${rev/maxR*100}%"></div></div>
    </div>`).join('');
}
function renderOrderRows(orders) {
  if(!orders.length) return `<div style="text-align:center;color:var(--muted);padding:30px">No orders found.</div>`;
  return orders.map(o=>`
    <div class="order-row">
      <span class="order-id">${o.id}</span>
      <span><span style="font-size:.88rem;color:var(--text)">${o.customer}</span><span class="order-restaurant">${o.restaurant} · ${o.items.map(i=>i.name).slice(0,2).join(', ')}${o.items.length>2?'...':''}</span></span>
      <span style="font-size:.82rem;color:var(--muted)">${o.restaurant}</span>
      <span class="order-amount">₹${Number(o.total).toLocaleString('en-IN')}</span>
...
function renderOrdersTable(filter) {
  ...
  const b=document.getElementById('orders-table-body');if(b)b.innerHTML=renderOrderRows(orders);
}
function filterOrders(filter,el) {
  ordersFilter=filter;
  document.querySelectorAll('.filter-btn').forEach(b=>b.classList.remove('active'));
  el.classList.add('active'); renderOrdersTable(filter);
}
async function updateOrderStatus(id,newStatus) {
  try { await updateOrderStatusDB(id,newStatus); showToast(`Order ${id} updated`); }
  catch(e) { const o=ORDERS_DB.find(o=>o.id===id);if(o)o.status=newStatus;renderOrdersTable(ordersFilter);showToast(`Order ${id} updated`); }
}
function renderMenuManager() {
  const grid=document.getElementById('menu-manager-grid');if(!grid)return;
  grid.innerHTML=MENU.map(item=>{
    const rest=RESTAURANTS.find(r=>r.id===item.restId);
    return `<div class="mm-card">
      <div class="mm-header"><span class="mm-emoji">${item.emoji}</span><div><div class="mm-name">${item.name}</div><div class="mm-rest">${rest?.name||''}</div></div></div>
      <div class="mm-price">₹${item.price}</div>
      <div class="mm-actions">
        <button class="mm-btn edit" onclick="editMenuItem('${item.firestoreId}',${item.price},'${item.name.replace(/'/g,"\\'")}')">Edit</button>
        <button class="mm-btn del"  onclick="deleteMenuItem('${item.firestoreId}','${item.name.replace(/'/g,"\\'")}')">Remove</button>
      </div>
    </div>`;
  }).join('');
}
async function editMenuItem(firestoreId,currentPrice,name) {
  const newPrice=prompt(`Edit price for "${name}" (current: ₹${currentPrice}):`);
  if (newPrice&&!isNaN(newPrice)&&Number(newPrice)>0) {
    try { await updateMenuItemDB(firestoreId,{price:parseInt(newPrice)}); showToast(`Price updated to ₹${newPrice}`); }
    catch(e) { showToast('Update failed — check Firebase config'); }
  }
}
async function deleteMenuItem(firestoreId,name) {
  if (!confirm(`Remove "${name}" from the menu?`)) return;
  try { await deleteMenuItemDB(firestoreId); showToast(`"${name}" removed`); }
  catch(e) { showToast('Delete failed — check Firebase config'); }
}
function showAddItemForm() { showToast('Add item form — coming soon!'); }
function renderManagerComplaints() {
  const all=COMPLAINTS_DB;
  const open=all.filter(c=>!c.resolved).length;
  const cnt=document.getElementById('open-complaints-count');if(cnt)cnt.textContent=`${open} open complaint${open!==1?'s':''}`;
  const list=document.getElementById('manager-complaints-list');if(!list)return;
  list.innerHTML=all.map(c=>`
    <div class="complaint-card">
      <div class="complaint-card-header">
        <div><div style="font-weight:600;font-size:.95rem;margin-bottom:2px">${c.customer}</div><div class="complaint-meta">${c.id} · Order ${c.orderId} · ${timeAgo(c.time)}</div></div>
        <span class="complaint-type ctype-${c.type}">${c.type}</span>
      </div>
      <div class="complaint-text">${c.text}</div>
      <div class="complaint-actions">
        ${c.resolved?`<span class="resolved-badge">Resolved</span>${c.response?`<span style="color:var(--muted);font-size:.82rem;margin-left:8px">${c.response}</span>`:''}`:
...
async function resolveComplaint(id) {
  try { await resolveComplaintDB(id,'Issue reviewed and resolved by our team.'); showToast(`Complaint ${id} resolved`); }
  catch(e) { showToast('Could not update — check Firebase config'); }
}
async function replyComplaint(id) {
  const msg=prompt('Enter your response to the customer:');
  if (msg) { try { await resolveComplaintDB(id,msg); showToast('Response sent'); } catch(e){showToast('Update failed');} }
}
function exportRevenue() {
  const rows=[['Order ID','Customer','Restaurant','Amount','Status','Pay Method','Date']];
  ORDERS_DB.forEach(o=>rows.push([o.id,o.customer,o.restaurant,`₹${o.total}`,o.status,o.payMethod||'',new Date(o.time).toLocaleDateString('en-IN')]));
  const csv=rows.map(r=>r.join(',')).join('\n');
  const a=document.createElement('a');
  a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));
  a.download='quickbite-revenue.csv'; a.click(); showToast('CSV exported!');
}

/* ══════════════════════════
   WAITER
══════════════════════════ */
function initWaiterDash() { renderWaiterOrders(); }
function renderWaiterOrders() {
  const orders=waiterFilter==='all'?ORDERS_DB:ORDERS_DB.filter(o=>o.status===waiterFilter);
  const grid=document.getElementById('waiter-orders-grid');if(!grid)return;
  if(!orders.length){grid.innerHTML=`<div style="color:var(--muted);grid-column:1/-1;text-align:center;padding:40px 0">No orders in this status.</div>`;return;}
  grid.innerHTML=orders.map(o=>`
    <div class="waiter-order-card">
      <div class="waiter-order-header"><span class="waiter-order-id">#${o.id}</span><span class="order-status status-${o.status}">${statusLabel(o.status)}</span></div>
...
async function waiterUpdate(id,status) {
  try { await updateOrderStatusDB(id,status); showToast(`Order #${id} updated`); }
  catch(e) { const o=ORDERS_DB.find(o=>o.id===id);if(o)o.status=status;renderWaiterOrders();showToast(`Order #${id} updated`); }
}
function filterWaiterOrders(filter,el) {
  ...
  el.classList.add('active'); renderWaiterOrders();
}

/* ══════════════════════════
   HELPERS
══════════════════════════ */
function statusLabel(s){return{pending:'⏳ Pending',preparing:'👨‍🍳 Preparing',out:'🛵 Out for Delivery',delivered:'✅ Delivered',cancelled:'❌ Cancelled'}[s]||s;}
function isToday(date){const d=new Date(date),n=new Date();return d.getDate()===n.getDate()&&d.getMonth()===n.getMonth()&&d.getFullYear()===n.getFullYear();}
function timeAgo(date){const diff=Date.now()-new Date(date).getTime();const mins=Math.floor(diff/60000);if(mins<1)return'just now';if(mins<60)return`${mins}m ago`;const hrs=Math.floor(mins/60);if(hrs<24)return`${hrs}h ago`;return Math.floor(hrs/24)+'d ago';}
function formatCard(input){let v=input.value.replace(/\D/g,'').slice(0,16);input.value=v.match(/.{1,4}/g)?.join('  ')||v;}
